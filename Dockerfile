...
WORKDIR /app
RUN corepack enable
# pnpm-workspace.yaml carries the security overrides, so it must be present for
# the frozen install to resolve consistently with the committed lockfile.
...
# Serve as the image's built-in unprivileged `nginx` user on an unprivileged
# port so the container never runs as root (satisfies trivy DS-0002).
