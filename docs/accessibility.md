...
- Informative images support authored alternative text; decorative images are serialized with an explicit empty `alt` value. A successful toolbar file conversion requires the author to make that informative-versus-decorative decision before insertion rather than silently treating omission as decorative intent.
...
- choosing meaningful alternative text for informative images and submitting an empty value only when the image is intentionally decorative in its authored context;
...
- [WCAG 2.2, Success Criterion 1.1.1 Non-text Content](https://www.w3.org/TR/WCAG22/#non-text-content) — informative non-text content needs an equivalent text alternative, while defined decorative exceptions may use an empty alternative.
- [WAI Images Tutorial](https://www.w3.org/WAI/tutorials/images/) and [Decorative Images guidance](https://www.w3.org/WAI/tutorials/images/decorative/) — image purpose determines whether meaningful alternative text or an explicit empty alternative is appropriate, so decorative classification remains an author decision.
...
- [Tiptap Link extension](https://tiptap.dev/docs/editor/extensions/marks/link) — the configured link extension provides commands and link behavior but no default keyboard shortcut; Inkspan's editor surface supplies its own `Ctrl/Cmd+K` binding.
