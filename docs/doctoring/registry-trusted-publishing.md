...
## Unified release version identity

The current repository publishes the npm tarball and Office wheel in one semantic-tagged GitHub Release. Registry publication makes historical independent package version numbers operationally ambiguous because PyPI versions are immutable and a later Inkspan release cannot safely pretend an old Office version is newly published from a new protected source.

ADR 0019 therefore adopts one release version train for trusted publication. The release tag, root npm package version, and Office package version must agree before registry publication. The historical Office `0.1.0` line is not rewritten; release preparation for the first trusted-publishing release must explicitly align the Office manifest to the selected Inkspan version through normal reviewed version/changelog work.

This decision intentionally prefers a simple acquisition/release identity over an independently incremented Office version cadence. A future independent Office release train requires a superseding ADR and separate tag/provenance/rollback authority.
...
A rerun of the same tag is acceptable only while the target registry version has not been published and all repository/GitHub release identity gates still hold. If a package version already exists, the release lane requires operator reconciliation rather than silently treating an upload skip as evidence that the current protected artifact reached that registry.

## GitHub Environment governance

The release workflow names environments `npm` and `pypi`. Environment configuration is external governance, not source code. Repository owners should restrict those environments to protected release branches/tags as supported, configure any required reviewers/separation of duties, and ensure the exact environment names match the npm/PyPI Trusted Publisher registrations.
