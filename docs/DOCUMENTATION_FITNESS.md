...
- protected-main React-free `@contextualwisdomlab/cwl-editor/text-position-selector` projection subpath for workers/adapters that do not need the root React capture surface;
...
1. Browser assurance, envelope identity routing, SafeClipboard, autosave lifecycle observation, security disclosure, toolbar shortcut accessibility metadata, SSR/native-form serialization, revision-scoped selection evidence, W3C text-position selector evidence, the React-free text-position selector subpath, and document-transition evidence are `implemented_on_protected_main` and must not be described as active-only work.
