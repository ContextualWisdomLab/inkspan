...
| W3C text-position selector evidence | active feature branch and its bounded doctoring/selection-lifecycle changes | `owned_by_separate_active_pr` | `implemented_on_active_pr`; not protected-main authority | The current active interoperability slice remains isolated from this canonical reconciliation and cannot be described as shipped until protected integration. |
...
1. W3C `TextPositionSelector` interoperability is implemented on a separate active feature branch and remains non-authoritative until its exact head is reviewed, reconciled with the live base, and protected-integrated.
2. Browser assurance, envelope identity routing, SafeClipboard, autosave lifecycle observation, security disclosure, toolbar shortcut accessibility metadata, SSR/native-form serialization, revision-scoped selection evidence, and document-transition evidence are `implemented_on_protected_main` and must not be described as active-only work.
...
PRD, TRD, Architecture, ADR, UML, conceptual ERD/data model, contracts, threat model, test strategy, operability, security disclosure, and traceability are `present_current` for the durable product and accepted/planned architecture decisions reconstructed from the conversation and live repository. Envelope identity routing, SafeClipboard, autosave lifecycle observation, the security disclosure lifecycle, toolbar shortcut accessibility metadata, SSR/native-form serialization, revision-scoped selection evidence, document-transition evidence, and cross-engine browser assurance are `implemented_on_protected_main`. A physical relational ERD is `not_applicable` because Inkspan deliberately owns no application persistence. The W3C text-position selector feature remains `implemented_on_active_pr` and cannot become shipped authority until its separate branch integrates.
