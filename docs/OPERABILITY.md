...
The protected workflow currently contains an ancestry check that is weaker than this exact-tip policy; Issue #96 and its test-first release-hardening line own the correction. Until that correction reaches protected `main`, an ancestry-only workflow success is insufficient to authorize publication, so release remains fail closed rather than weakening this policy to match the implementation gap.
