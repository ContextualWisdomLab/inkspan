...
SafeClipboard and the browser-realistic release assurance are shipped on protected `main`. The protected gate uses dependency-locked **Playwright 1.62.0** Chromium, Firefox, and WebKit projects and binds every result to one **exact source head**, one browser-test lock digest, one corpus version, and one **fresh run identity**. On the tag release path it additionally tests the exact **packed npm artifact** produced by the release build and records the artifact's byte-derived **SHA-256**. Feature-branch or predecessor-head success does not substitute for a fresh tagged release-candidate run.
