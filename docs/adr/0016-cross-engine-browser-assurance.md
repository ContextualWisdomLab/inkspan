...
SafeClipboard and the cross-engine assurance gate are now integrated on protected `main`. Issue #66 established the browser-realistic differential release gate. The architectural decision is how browser-semantic differences are admitted into a release without turning engine disagreement into either an unbounded compatibility promise or an excuse to normalize away a security defect.
...
The release path additionally verifies the packed npm artifact rather than relying only on source-tree execution. Bounded browser evidence is retained with source/package/run identity while broad Playwright result directories are excluded from release evidence.
...
The protected gate starts from the integrated SafeClipboard boundary and does not change existing document-envelope or persistence migration semantics.

## Verification

Issue #66 defined the test-first implementation acceptance. Protected `main` now contains a permanent differential oracle, historical RED lineage, real Playwright Chromium/Firefox/WebKit execution through the supported paste pipeline, deterministic fresh-run/corpus/lock/package evidence, representative performance bounds, packed-artifact release verification, bounded evidence retention, and exact-head CI/security/package contracts. Any source movement still invalidates predecessor evidence and requires the exact current head to re-prove the gate.

Release acceptance additionally requires zero valid unresolved findings, applicable repository review/policy gates, and successful tag-source execution of the packed-artifact browser path. The canonical documentation and test strategy continue to state that jsdom-only evidence is not real-engine conformance and that differences are never normalized merely to make engines agree.

## Rollback or supersession

Disabling a required engine, removing packed-artifact verification, weakening evidence identity, or loosening difference admission is a security/release-policy change requiring a superseding ADR and new threat analysis.

Supersession is acceptable only if a later harness provides equivalent or stronger real-engine coverage and preserves exact-head, fail-closed, synthetic-fixture, packed-artifact, and reviewed-difference boundaries.
