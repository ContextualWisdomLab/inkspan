...
describe('DOCX open/import contract', () => {
  it.each([
    ['ArrayBuffer', (bytes: Uint8Array) => bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength)],
    ['Uint8Array', (bytes: Uint8Array) => bytes],
    ['Blob', (bytes: Uint8Array) => new Blob([bytes])],
  ])('imports bounded document content from %s', async (
    _label: string,
    source: (bytes: Uint8Array) => ArrayBuffer | Uint8Array | Blob,
  ) => {
    const relationships =
  ...
  it('validates the complete imported document before one atomic editor mutation', async () => {
    const validateDocumentJson = vi.fn(() => ({ valid: true }));
    const setDocumentJson = vi.fn(() => true);
    ...
    const imported = validateDocumentJson.mock.calls[0]?.[0] as DocxJsonContent;
  ...
  it('does not mutate the editor when schema validation rejects the import', async () => {
    ...
    const operation = openDocx(
      {
        validateDocumentJson: () => ({ valid: false, issues: [] }),
        setDocumentJson,
      },
      createDocx(),
    );
